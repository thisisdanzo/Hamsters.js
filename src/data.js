...
*/
export function mergeOutputData(task, threadId, results) {
...
*/
export function prepareOutput(task) {
...
*/
export function generateReturnObject(task, output) {
  return {
...
* @param {object} buffer - Buffer to convert
*/
export function typedArrayFromBuffer(dataType, buffer) {
  const types = {
    'uint32': Uint32Array,
    'uint16': Uint16Array,
    'uint8': Uint8Array,
...
*/
export function locateBlobBuilder() {
  ...
  return errorLog('Environment does not support data blobs!');
}
...
*/
export function randomArray(count, onSuccess) {
  var randomArray = [];
  while(count > 0) {
...
*/
export function determineSubArrayIndexes(array, n) {
  let i = 0;
...
*/
export function getSubArrayUsingIndex(array, index) {
  if(array.slice) {
